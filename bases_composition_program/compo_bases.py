...
def get_coding_corresponding (fasta_file_corr) :
    ...
    """
    This function allows to read the corresponding fasta file and to get the corresponding
    sequence in a list.
...
def get_genome_len (fasta_file) :
    ...
    """
    This function allows to get the lenght of the genome in reference to the alignment file.
       
    Input : alignment fasta file
    Output : the genome lenght in integer format 
    """
...
def get_col (genome_len , step, win_size ) :
    ...
    """
    This function allows to get the number of window used in the bsw program.
    
    Input : the genome length, the step used in bsw program, the window size used in the bsw program
...
def get_base_per_window (base_princ, base_2, base_3, base_4, win_nucl_real, all_seq , num_seq, list_corr) :

    """
    This function allows to get the % composition in each base for each analyse window
    
    Input : bases, windows positions, sequences from alignment fasta, corresponding list
    Output : a dictionnary for each base with % per window 
    """	
...
def main () :
    ...
    for num_seq in range(len(all_seq)) :
        ...
        nb_A = get_base_per_window ( "A", "T", "C", "G", win_nucl_real, all_seq, num_seq, list_corr)
        
        nb_C = get_base_per_window ( "C", "T", "A", "G", win_nucl_real, all_seq, num_seq, list_corr)
        nb_G = get_base_per_window ( "G", "T", "C", "A", win_nucl_real, all_seq , num_seq, list_corr)
        nb_T = get_base_per_window ( "T", "A", "C", "G", win_nucl_real, all_seq, num_seq, list_corr)
