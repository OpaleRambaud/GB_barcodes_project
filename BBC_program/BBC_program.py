...
def main():
    ...
    l_row_csv = bc.bootstrap_per_windows (args.window_size,args.step,nb_col,data_sorted, nb_of_row, genome_len) 
    #color = bc.set_color_for_barcodes()
    scale_or_not = int(args.scale_or_not)
    graduation = int(args.graduation_value)
    bc.barcodes_plot (data_sorted, dic_name2,l_row_csv, nb_of_row, scale_or_not, graduation, genome_len) 
    print("End of the analyse. Your barcodes are all done ! You can see them in the current field.") 
    
    os.remove("name_data_filtered_sup_sorted.csv")
    os.remove("data_filtered_sup_sorted.csv")
    os.remove("data_filtered_sup_70.csv")
